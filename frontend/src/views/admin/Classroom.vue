<template>
  <div class="manage-container">
    ...
    <el-table :data="tableData" border v-loading="loading">
      <el-table-column prop="roomId" label="教室编号" width="120" />
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="400px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="80px">
        <el-form-item label="教室编号" prop="roomId" v-if="!isEdit">
          <el-input v-model="form.roomId" placeholder="请输入教室编号" maxlength="10" />
        </el-form-item>
        <el-form-item label="教室名称" prop="roomName">
          <el-input v-model="form.roomName" placeholder="例如：信息楼101" maxlength="20" />
        </el-form-item>
...
const rules = {
  roomId: [
    ...
    { max: 10, message: '教室编号不超过10位', trigger: 'blur' }
  ],
  roomName: [
    { required: true, message: '请输入教室名称', trigger: 'blur' },
    { max: 20, message: '教室名称不超过20位', trigger: 'blur' }
  ],
  capacity: [
...
async function fetchData() {
  loading.value = true
  try {
    const res = await getClassroomPage(page.current, page.size, searchForm.keyword)
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm('确定删除该教室吗？', '提示', { type: 'warning' })
    await deleteClassroom(row.roomId)
